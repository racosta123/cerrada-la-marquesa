...
const WORKER = CONFIG.workerUrl;        // ej. https://la-marquesa-proxy.acosta4770.workers.dev
const DOORS = [
...
async function invitarPersona(p, btn){
  ...
  try {
    ...
    const texto = `Hola, te invito a registrarte en la app de Cerrada La Marquesa para ${destino}. Abre este enlace (vence en 72 h):`;
    await compartirInvitacion({ title:'Cerrada La Marquesa', text: texto, url },
...
$('#saveFamiliarBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    const url = new URL('registro.html', location.href).href + '#' + r.token;
    const texto = `Hola ${nombre}, te invito a registrarte en la app de Cerrada La Marquesa como parte de mi familia. Abre este enlace (vence en 72 h):`;
    await compartirInvitacion({ title:'Cerrada La Marquesa', text: texto, url }, { titulo: nombre, sub:'Familiar' });
  } catch(e){
...
async function reenviarFamiliar(f, btn){
  ...
  try {
    ...
    const url = new URL('registro.html', location.href).href + '#' + r.token;
    const texto = `Hola ${f.nombre}, te invito a registrarte en la app de Cerrada La Marquesa como parte de mi familia. Abre este enlace (vence en 72 h):`;
    await compartirInvitacion({ title:'Cerrada La Marquesa', text: texto, url }, { titulo: f.nombre, sub:'Familiar' });
  } catch(e){
...
async function construirReportePDF(){
    ...
    doc.text('Cerrada La Marquesa', M, y + 42);
    ...
    for (let p = 1; p <= totalPages; p++){
      ...
      doc.text('Reporte financiero oficial · Cerrada La Marquesa · Transparencia vecinal', M, H - 24);
      doc.text(`${folio} · Página ${p} de ${totalPages}`, W - M, H - 24, { align:'right' });
    }

    return { doc, filename: `Finanzas-La-Marquesa-${label.replace(' ','-')}.pdf` };
}
...
async function compartirReportePDF(){
  ...
  try {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })){
      await navigator.share({ files: [file], title: 'Reporte financiero · Cerrada La Marquesa' });
...
$('#reciboWaBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (navigator.canShare && navigator.canShare({ files:[file] })){
      await navigator.share({ files:[file], title:`Recibo ${mov.folioRecibo} · Cerrada La Marquesa` });
...
function construirReporteCasaPDF(casa, movs){
  ...
  for (let p = 1; p <= totalPages; p++){
    doc.setPage(p);
    doc.setFont('helvetica','normal'); doc.setFontSize(7); doc.setTextColor(...PDF.notes);
    doc.text('Historial de pagos oficial · Cerrada La Marquesa · Transparencia vecinal', M, H - 24);
    doc.text(`${casa} · Página ${p} de ${totalPages}`, W - M, H - 24, { align:'right' });
  }

  return { doc, filename: `Pagos-${casa.replace(/\s+/g,'-')}-La-Marquesa.pdf` };
}
